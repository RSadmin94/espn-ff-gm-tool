...
export async function getAnyActiveEspnCredentials(leagueId?: string): Promise<EspnCreds | undefined> {
  const db = await getDb();
  if (!db) return undefined;
  ...
  const rows = await db
    .select()
    .from(leagueConnections)
    .where(leagueId ? and(baseCondition, eq(leagueConnections.leagueId, leagueId)) : baseCondition)
    .orderBy(desc(leagueConnections.updatedAt))
    .limit(1);
  if (!rows.length) return undefined;
  const row = rows[0];
