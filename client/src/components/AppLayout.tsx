...
// ─── Compact League Switcher for Header ──────────────────────────────────────
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-accent/40 hover:bg-accent/70 transition-colors text-left group whitespace-nowrap">
        ...
        {!myLeagues.isLoading &&
          leagues.map((league) => {
            ...
            return (
              ...
              >
                <span className="text-sm flex-shrink-0">
                  {HEADER_PROVIDER_EMOJI[league.provider] ?? "🏆"}
                </span>
